...
static float ref_wheels_chassis_contig_rot = 8.0;					// reference to wheels angular speed when chassis contiguously rotates	[rad/s]
...
void std_chassis_control_loop() {
	...
	std_chassis.x[4] = ((float) (((int) ((get_yaw_gimbal_motor_measures()->contig_ang_pos_rad - CHASSIS_YAW_ENCODER_ZERO_OFFSET) * 180/pi)) % 360) * pi/180);
	if (std_chassis.x[4] > pi)
		std_chassis.x[4] -= 2*pi;
	else if (std_chassis.x[4] < -pi)
		std_chassis.x[4] += 2*pi;
	
	
	/* take references from keyboard/joystick */
	float ref_rc_fwd_bwd_fl_br;				// reference to wheels from remote controller for forward/backward movement of front-left and back-right wheels [rad/s]
	float ref_rc_fwd_bwd_fr_bl;				// reference to wheels from remote controller for forward/backward movement of front-right and back-left wheels [rad/s]
	float ref_rc_left_right_fl_br;		// reference to wheels from remote controller for left/right of front-left and back-right wheels [rad/s]
	float ref_rc_left_right_fr_bl;		// reference to wheels from remote controller for left/right of front-right and back-left wheels [rad/s]
	float ref_chassis_rot;						// reference to wheels for the rotation of the chassis w.r.t. the gimbal [rad/s]
	
	if (robot_control_mode == REMOTE_CONTROLLER_MODE || robot_control_mode == FIXED_GIMBAL_PC_MODE)
	{
		if (robot_control_mode == REMOTE_CONTROLLER_MODE) {
			// take forward/backward and right/left references
			ref_rc_fwd_bwd_fl_br 		= ((float) get_remote_control_point()->rc.ch[3] / MAX_RC_TILT) * max_ref_wheels_lin;
			ref_rc_fwd_bwd_fr_bl 		= ref_rc_fwd_bwd_fl_br;
			ref_rc_left_right_fl_br = ((float) get_remote_control_point()->rc.ch[2] / MAX_RC_TILT) * max_ref_wheels_lin;
			ref_rc_left_right_fr_bl = ref_rc_left_right_fl_br;
		...
		else if (robot_control_mode == FIXED_GIMBAL_PC_MODE) {
			
			ref_rc_fwd_bwd_fl_br 		= ((float) get_remote_control_point()->rc.ch[3] / MAX_RC_TILT) * max_ref_wheels_rot * sin(- std_chassis.x[4] + pi/4);
			ref_rc_fwd_bwd_fr_bl 		= ((float) get_remote_control_point()->rc.ch[3] / MAX_RC_TILT) * max_ref_wheels_rot * cos(- std_chassis.x[4] + pi/4);
			ref_rc_left_right_fl_br = ((float) get_remote_control_point()->rc.ch[2] / MAX_RC_TILT) * max_ref_wheels_rot * sin(+ std_chassis.x[4] + pi/4);
			ref_rc_left_right_fr_bl = ((float) get_remote_control_point()->rc.ch[2] / MAX_RC_TILT) * max_ref_wheels_rot * cos(+ std_chassis.x[4] + pi/4);
	...
	else if (robot_control_mode == CHASSIS_FOLLOW_GIMBAL_PC_MODE)
	{
		// take forward/backward and right/left references
		compute_weights_WASD_keys(dt_chassis);
		
		// forward/backward wheels 1 and 3
		ref_rc_fwd_bwd_fl_br = 0.0;
		ref_rc_fwd_bwd_fl_br += max_ref_wheels_lin * weight_fwd_key;
		ref_rc_fwd_bwd_fl_br -= max_ref_wheels_lin * weight_bwd_key;
		// forward/backward wheels 2 and 4
		ref_rc_fwd_bwd_fr_bl = ref_rc_fwd_bwd_fl_br;
		
		// right/left wheels 1 and 3
		ref_rc_left_right_fl_br = 0.0;
		ref_rc_left_right_fl_br += max_ref_wheels_lin * weight_right_key;
		ref_rc_left_right_fl_br -= max_ref_wheels_lin * weight_left_key;
		...
		if (is_key_pressed(KEY_SHIFT)) {
			...
			ref_rc_fwd_bwd_fl_br *= gain_decr_speed;
			ref_rc_fwd_bwd_fr_bl *= gain_decr_speed;
			ref_rc_left_right_fl_br *= gain_decr_speed;
			ref_rc_left_right_fr_bl *= gain_decr_speed;
		}
	...
	else if (robot_control_mode == FIXED_GIMBAL_PC_MODE)
	{
		...
		ref_rc_fwd_bwd_fl_br = 0.0;
		ref_rc_fwd_bwd_fr_bl = 0.0;
		ref_rc_fwd_bwd_fl_br += max_ref_wheels_rot * sin(- std_chassis.x[4] + pi/4) * weight_fwd_key;
		ref_rc_fwd_bwd_fr_bl += max_ref_wheels_rot * cos(- std_chassis.x[4] + pi/4) * weight_fwd_key;	
		ref_rc_fwd_bwd_fl_br -= max_ref_wheels_rot * sin(- std_chassis.x[4] + pi/4) * weight_bwd_key;
		ref_rc_fwd_bwd_fr_bl -= max_ref_wheels_rot * cos(- std_chassis.x[4] + pi/4) * weight_bwd_key;
		
		// right/left wheels 1, 2, 3, 4
		ref_rc_left_right_fl_br = 0.0;
		ref_rc_left_right_fr_bl = 0.0;
		ref_rc_left_right_fl_br += max_ref_wheels_rot * sin(std_chassis.x[4] + pi/4) * weight_right_key;
		ref_rc_left_right_fr_bl += max_ref_wheels_rot * cos(std_chassis.x[4] + pi/4) * weight_right_key;
		ref_rc_left_right_fl_br -= max_ref_wheels_rot * sin(std_chassis.x[4] + pi/4) * weight_left_key;
		ref_rc_left_right_fr_bl -= max_ref_wheels_rot * cos(std_chassis.x[4] + pi/4) * weight_left_key;
		
		// if SHIFT is pressed, go faster
		if (is_key_pressed(KEY_SHIFT)) {
	...
	std_chassis.ref[0] = + ref_rc_fwd_bwd_fl_br + ref_rc_left_right_fl_br - ref_chassis_rot;
	std_chassis.ref[1] = - ref_rc_fwd_bwd_fr_bl + ref_rc_left_right_fr_bl - ref_chassis_rot;
	std_chassis.ref[2] = - ref_rc_fwd_bwd_fl_br - ref_rc_left_right_fl_br - ref_chassis_rot;
	std_chassis.ref[3] = + ref_rc_fwd_bwd_fr_bl - ref_rc_left_right_fr_bl - ref_chassis_rot;
