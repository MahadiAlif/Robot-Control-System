		...
		else if ( rc_ctrl->rc.s[0] == RC_SW_MID )
		{
			if (is_key_falling_edge(KEY_R))
			{
				if (robot_control_mode == CHASSIS_FOLLOW_GIMBAL_PC_MODE)
					new_robot_control_mode = (IS_STD) ? FIXED_GIMBAL_PC_MODE : BALANCING_90_DEGREE_PC_MODE;
				else if (robot_control_mode == (IS_STD) ? FIXED_GIMBAL_PC_MODE : BALANCING_90_DEGREE_PC_MODE)
